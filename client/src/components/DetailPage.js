...
const ColCardBox = ({Title, Poster, Type, ShowDetail, DetailRequest, ActivateModal}) => {
    ...
    const clickHandler = () => {
        ...
        fetch(`http://www.omdbapi.com/?i=apikey=${process.env.OMDB_KEY}`)
        .then(resp => resp)
        .then(resp => resp.json())
        .then(response => {
            DetailRequest(false);
...
const MovieDetail = ({Title, imdbRating, Year, Poster, Rated, Genre, Plot, Actors}) => {
    return (
        <Row>
            <Col span={11}>
                <img 
...
function App() {
    ...
    useEffect(() => {
        ...
        fetch(`http://www.omdbapi.com/?s=${q}&apikey=${process.env.OMDB_KEY}`)
        .then(resp => resp)
        .then(resp => resp.json())
        .then(response => {
